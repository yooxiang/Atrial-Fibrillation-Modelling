...
#%%
x = RunState(50,4000,90,90,10,1,0.55,5,30,3)
#%%save x data
np.save('DataFor17.11',x)
#%%
MovieNodes(x)
#%%
writergif = animation.PillowWriter()
MovieNodes(x).save('RunState(50,4000,90,90,10,1,0.55,5,30,3).gif',writer=writergif)
#%%average of in and out vectors
Vectors = Resultant_Vectors(x[5],x[6],x[2],x[1],x[7],5,90)
#%%only out vectors
Vectors1= Resultant_Vectors(x[5],x[6],x[2],x[1],x[7],5,90,outv=True)
#%%only in vectors
Vectors2= Resultant_Vectors(x[5],x[6],x[2],x[1],x[7],5,90,inv=True)
#%%movie for vectors
X=[]
Y=[]
U=[]
...
#%%
writergif = animation.PillowWriter()
anim.save('RunState(50,4000,90,90,10,1,0.55,5,30,3)_V.gif',writer=writergif)
#%%frame display for vectors
...
fig, ax = plt.subplots(1,1)
Q = ax.quiver(X, Y, U, V, pivot='mid', angles='xy', scale_units='xy',scale=1)
ArrowFrameMicro(16,Q)
#plt.savefig('Vavg',dpi=1000)
#%%movie for vectors1
X=[]
Y=[]
U=[]
...
def update_quiver(num,Q):
    ...
    Q.set_UVC(U,V)
    for i in range(len(x[1])):
        U[i]=Vectors1[num][i][0]
        V[i]=Vectors1[num][i][1]
    Q.set_UVC(U,V)
    return Q,

anim1 = animation.FuncAnimation(fig, update_quiver,frames=len(Vectors1),fargs=(Q,),
                               interval=500, blit=True)
fig.tight_layout()
#%%
writergif = animation.PillowWriter()
anim1.save('RunState(50,4000,90,90,10,1,0.55,5,30,3)_V1.gif',writer=writergif)
#%%frame display for vector1
def ArrowFrameMicro(num,Q):
    U=[0]*len(x[1])
    V=[0]*len(x[1])
    Q.set_UVC(U,V)
...
def update_quiver(num,Q):
    ...
    Q.set_UVC(U,V)
    for i in range(len(x[1])):
        U[i]=Vectors2[num][i][0]
        V[i]=Vectors2[num][i][1]
    Q.set_UVC(U,V)
    return Q,
...
#%%
writergif = animation.PillowWriter()
anim1.save('RunState(50,4000,90,90,10,1,0.55,5,30,3)_V2.gif',writer=writergif)
#%%frame display for vector1
def ArrowFrameMicro(num,Q):
    U=[0]*len(x[1])
    V=[0]*len(x[1])
...
Q = ax.quiver(X, Y, U, V, pivot='tip', angles='xy', scale_units='xy',scale=1)
ArrowFrameMicro(16,Q)
plt.savefig('VIn',dpi=1000)
#%%data for pixelated vectors
VectorPixels=PixelatedVectors(x,Vectors,9,9)
...
#%%
writergif = animation.PillowWriter()
anim.save('RunState(50,4000,90,90,10,1,0.55,5,30,3)_Vpixel9x9.gif',writer=writergif)
#%%pixelated grid 9x9
xpixel=Pixelation(x,9,9)
#%%saving the pixel movie
writergif = animation.PillowWriter()
MoviePixels(xpixel).save('RunState(50,4000,90,90,10,1,0.55,5,30,3)_Vpixel9x9.gif',writer=writergif)
