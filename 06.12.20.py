# -*- coding: utf-8 -*-
"""
Created on Sun Dec  6 12:47:35 2020
...
def AFIdentify(vector_data, coord, percentile):
    ...
    return AFinc
#%%observing sd and mean of divergence over range of epsilon for averaged vector
data_all_a=[]
divAll_a=[]
mean_a=[]
std_a=[]
...
for e in epsilon:
    ...
    for time_step in range(len(pxv_o[0])):
        ...
        for cell in range(len(divergence_matrix[0])):
            div_all.append(divergence_matrix[0][cell])
    divAll_a.append(div_all)
            
    mean_o=np.mean(div_all)
    std_o=np.std(div_all)
    mean_a.append(mean_o)
    std_a.append(std_o)
#%%removing the edge points of the above
data_all_ane=[]
divAll_ane=[]
pxv_All_ane=[]
mean_ane=[]
...
counts,bin_edges = np.histogram(divAll_a[2],20)
plt.hist(divAll_ane[2],20)
#%%    
plt.plot(epsilon,std_ane,'o')
...
#full agreement except from upper and lower boundaries
...
for e in epsilon:
    ...
    for time_step in range(len(pxv_o[0])):
        ...
        for cell in range(len(divergence_matrix[0])):
            div_all.append(divergence_matrix[0][cell])
    #del div_all[:64]#remove upper and lower boundaries
    #del div_all[4032:]
    ...
    std.append(std_o)
#%%
plt.plot(epsilon,std,'o')
plt.xlabel(r'Epsilon,$\epsilon$')
plt.ylabel('Standard deviation of divergence')
plt.figure()
plt.plot(epsilon,mean,'x')
plt.xlabel(r'Epsilon,$\epsilon$')
plt.ylabel('Mean of divergence')
plt.tight_layout()