# -*- coding: utf-8 -*-
"""
Created on Mon Jan 18 19:25:49 2021
...
#for this version of the code we change it so that it provides the top 5 
#divergent points
def AFIdentifyBoth(vector_data, coord, data,linfit,error,dim_m,dim_n):
    #dim_m is microscopic dimension
    #dim_n is coarse grained dimension
    period=data[8][8]
    duration=data[8][0]
    epsilon=data[8][6]
    x_size=max([i[0] for i in coord])+1 #sets x length of grid
    y_size =max([i[1] for i in coord])+1 #sets y length of grid
    sfactor=dim_n/dim_m
    mdiv=int(dim_n**2*0.01)#gives how many of the top divergent points to be 
    #considered
    ...
    if len(AFall)==0:
        AFall=[[[0,0],0]]*mdiv
    return AFall,std,maxstdall
#%%mathematical way of determining how many sd away for fit parameters to use
def AFIdentifySD(vector_data, coord, data,linfit,error,dim_m,dim_n,sd):
    period=data[8][8]
    epsilon=data[8][6]
    x_size=max([i[0] for i in coord])+1 #sets x length of grid
    y_size =max([i[1] for i in coord])+1 #sets y length of grid
    ...
    cs=0
    maxstdall=(linfit[0]+error[0]*sd)*epsilon+(linfit[1]+error[1]*sd)
    div_all=[]#list with divergence data for all times
    for time_step in range(len(vector_data)):
        divergence_matrix = divergence(vector_data[time_step],coord,y_size,x_size)
        for cell in range(len(divergence_matrix[0])):
            div_all.append(divergence_matrix[0][cell])
    stdslice=np.std(div_all)
    if stdslice>maxstdall:
        cs+=1
    ...
    std_p=[]#all 
    for p in range(period):                
        div_p=div_all[p*x_size*y_size:(p+1)*x_size*y_size]
        std_p.append(np.std(div_p)*sfactor)
...
#%%
def focal_quality_indicator(data,coordinates,x_dim,y_dim,x_spacing,y_spacing,middle_index,time,velocity_conduction,n_time_steps):
    #data is from first ouput of PixelatedVectors
    vector_circle = [] #this is where the vectors are recorded
    cell_centres = [] #this is where the coodinates of the centres of cells are stored
    
    data = data[time:]
    
    def y_shortest_distance(y_0,y_1,y_spacing,y_dim, direction = False):
        distance = 0
        long_distance = np.abs(y_0 - y_1)
        if direction == True:
            long_distance = y_0 - y_1
    
        if y_0 >= y_1:
            short_distance = np.abs((y_0 - y_dim*y_spacing) - y_1)
            if direction == True:
                short_distance = y_0 - y_dim*y_spacing - y_1
    
        if y_1 > y_0:
            short_distance = np.abs((y_1  - y_dim*y_spacing) - y_0)
            if direction == True:
                short_distance = y_1  - y_dim*y_spacing - y_0
    
        if np.abs(short_distance) < np.abs(long_distance):
    ...
    def index_to_coor(index,x_dim,y_dim,x_spacing, y_spacing):
        y_coord  = y_spacing*(index//x_dim )
        x_coord = x_spacing*(index - y_dim*(index//x_dim))
        return x_coord,y_coord

    def coor_to_index(x_coord, y_coord, x_dim, x_spacing, y_spacing):
        index = (x_coord/x_spacing) + (y_coord/y_spacing)//x_dim
        return index

    for i in range(x_dim*y_dim):
        vector_circle.append([0,0])
        centres = index_to_coor(i,x_dim,y_dim,x_spacing,y_spacing)
        cell_centres.append([centres[0]+1,centres[1]+1])
    
    def check_valid(time_step,index,velocity_conduction,middle_index): #check if a cell is within the ring AND hasn't been chosen before
        state = False
        cell_centre_x = cell_centres[index][0]
        cell_centre_y = cell_centres[index][1]
    
        if time_step*velocity_conduction > np.sqrt((cell_centre_x - coordinates[middle_index][0])**2+ (y_shortest_distance(cell_centre_y,coordinates[middle_index][1],y_spacing,y_dim))**2):
            if vector_circle[index][0] == 0 and vector_circle[index][1] == 0:
                state = True
        return state
    ...
    for time_step in range(1,n_time_steps):
        for cell in range(x_dim*y_dim):
            if check_valid(time_step,cell,velocity_conduction, middle_index) == True:
                max_count = max_count + 1
                vector_circle[cell] = data[time_step][cell]

    #now generate maxiumal divergent field    
    vector_perfect = []
    cell_centres = []
    for i in range(x_dim*y_dim):
        vector_perfect.append([0,0])
        centres = index_to_coor(i,x_dim,y_dim,x_spacing,y_spacing)
        
        cell_centres.append([centres[0]+x_spacing/2,centres[1]+y_spacing/2])

    for i in range(x_dim*y_dim):
        angle=0
        x_centre = coordinates[middle_index][0]
        y_centre = coordinates[middle_index][1]
        if cell_centres[i][0] == x_centre:
            if cell_centres[i][1] - x_centre > 0:
                angle = np.pi/2
                
            if cell_centres[i][1] - x_centre < 0:
                angle = -np.pi/2
            if cell_centres[i][1] - x_centre == 0:
                vector_perfect[i][0] = 0
                vector_perfect[i][1] = 0
                
        else:
            angle = math.atan((y_shortest_distance(cell_centres[i][1],y_centre,y_spacing,y_dim))/(cell_centres[i][0] - x_centre))

        if cell_centres[i][0] - x_centre < 0 and y_shortest_distance(cell_centres[i][1],y_centre,y_spacing,y_dim, True) < 0:
            vector_perfect[i][0] = -np.cos(angle)
            vector_perfect[i][1] = np.sin(angle)    
    ...
    dot_summation = []
    for i in range(len(vector_perfect)):
        dot_product = np.dot(vector_perfect[i],vector_circle[i])
        dot_summation.append(dot_product)
    
    quality = sum(dot_summation)/max_count
    #VectorMovie([vector_circle],coordinates,0)
    #VectorMovie([vector_perfect],coordinates,0)
    #print(quality)
    return quality
